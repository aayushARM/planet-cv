...
from tensorflow.python.keras.layers import Input, Dense, Activation, BatchNormalization, \
    MaxPooling2D, GlobalAveragePooling2D, GlobalMaxPooling2D, Conv2D, add
...
def SEResNet(input_shape=None,
    ...
    # Ensure that the model takes into accounttf.python.keras.backend
...
def _resnet_bottleneck_block(input, filters, k=1, strides=(1, 1)):
    ...
    if strides != (1, 1) or K.int_shape(init)[channel_axis] != bottleneck_expand * filters * k:
