...
from tensorflow.python.keras.layers import concatenate, GlobalAveragePooling2D, AveragePooling2D, BatchNormalization, \
                                            Conv2D, Dense, Dropout, MaxPooling2D, Activation, Lambda
...
class DenseNetCreator:
    ...
    def _transition_block(self, ip, nb_filter):
        ...
        x = BatchNormalization()(ip)
        x = Activation(activation='relu')(x)
        x = Conv2D(int(nb_filter * self.compression), (1, 1), kernel_initializer='he_normal',
                   padding='same', use_bias=False, **self.conv_kwargs)(x)
        x = AveragePooling2D((2, 2), strides=(2, 2), data_format=self.data_format)(x)

        return x

    def __call__(self):
        """ Builds the network. """
        x = Conv2D(self.nb_filter, self.initial_kernel, kernel_initializer='he_normal', padding='same',
                   strides=self.initial_strides, use_bias=False, **self.conv_kwargs)(self.img_input)

        if self.subsample_initial_block:
            x = BatchNormalization()(x)
            x = Activation(activation='relu')(x)
            x = MaxPooling2D((3, 3), data_format=self.data_format, strides=(2, 2), padding='same')(x)
        ...
        x = Dense(self.nb_classes, activation='sigmoid')(x)
